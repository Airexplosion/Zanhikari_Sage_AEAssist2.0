...
public class 贤者能力技_自动单奶 : ISlotResolver
{
    ...
    {   //QT奶人关了就过
        ...
        //
        var 海马目标血量 = PartyHelper.CastableAlliesWithin30//周围30米
            .Where(r => r.CurrentHealth > 0 && r.CurrentHealthPercent <= 贤者设置.实例.单海马阈值 && r.IsTank() && //血量大于0 且 低于设置的阈值
                                              !r.HasAura(AurasDefine.LivingDead) && !r.HasAura(AurasDefine.WalkingDead) && !r.HasAnyAura(排除buff, 3000))//且 不具有几个buff 且不具有list中的buff 3秒
            .OrderBy(r => r.CurrentHealthPercent)//排序
            .FirstOrDefault();
